...
void Board::searchLeaf(const int index, const Board &baseboard, int minloc, int basescore)
{
    Board b;
...
void Board::search(const int index, const Board &baseboard, int minloc, int maxloc /*not include*/, int basescore, int depth)
{
    if (depth == MOVE - 2)
    {
        //显示正在搜的第一步和第二步的位置
        ...
        std::cout << index << " "
                  << "进度条:" << MoveBuf[0] << "_" << MoveBuf[1] << std::endl;
    }
    if (depth == 1)
    {
        searchLeaf(index, baseboard, minloc, basescore);
        return;
    }
    Board b;
    for (int loc = minloc; loc < maxloc; loc++)
    {
        ...
        search(index, b, loc, H * W, score, depth - 1);
    }
}